    ...
    class Build : CommandBase
    {
        ...
        public override bool doProcess(string[] args)
        {
            ...
            if (target != null)
            {
                ...
                if (!pData.canBuild)
                {
                    StarshipServer.sendGlobalMessage("^#f75d5d;" + pData.name + " has had their build rights revoked!");
                }
                else
                {
                    StarshipServer.sendGlobalMessage("^#6cdb67;" + pData.name + " has been granted build rights on the server.");
                }
                return true;
            }
            else
